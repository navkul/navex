...
## Lifecycle findings
- The stale overlay rows were not primarily a Swift-helper problem. They came from treating `registry.json` as append-only state and only resetting it indirectly when the daemon restarted.
- The wrapper process is the cleanest liveness anchor for an interactive Codex session because it begins before the first hook fires and ends exactly when the interactive session ends.
- Persisted rows without a live launcher PID are not trustworthy enough to keep. For this product, it is better to drop an unprovable session than to show a stale one in the overlay.
- Once the daemon owns session removal explicitly, the snapshot can safely model "all live sessions" instead of "all sessions that last stopped".

# April 22, 2026 absolute-hook launcher findings

## Hook findings
- The linked global `navex` bin is the right absolute hook target when it resolves to the current repo's `dist/cli.js`.
- That path is cleaner than repeating both the Node executable and the repo-local CLI path in every hook entry.
- A guarded fallback still matters because `install` can also be run directly from `node dist/cli.js`, where no trusted linked `navex` bin may exist yet.

# April 22, 2026 hook-path install findings

## Hook findings
- Codex hook execution cannot assume the user's interactive shell PATH.
- For this tool, generated absolute hook commands are more reliable than asking the user to depend on `navex` being discoverable by name.
- The right setup contract is:
  - wrapper can stay name-based in the user's shell
  - hook commands should be absolute and machine-specific

# April 22, 2026 navex cleanup findings

## Cleanup findings
- The broad old-name compatibility layer was useful for unblocking a broken live install, but it was not the right steady-state design.
- The clean end state is simpler:
  - one package/bin name
  - one env var family
  - one state root
- Keeping only a narrow one-time legacy root migration preserves upgrade safety without making the runtime harder to reason about.

# April 22, 2026 navex rename findings

## Rename findings
- A clean rename here needs more than package metadata. The wrapper command, hook commands, helper binary name, state directory, and visible helper title all have to move together or the product feels half-renamed.
- Keeping a temporary `codex-beacon` bin alias and legacy `CODEX_BEACON_*` env fallbacks is the pragmatic migration path because existing local shell snippets and hook files may still point at the old name.
- `.gitignore` is not enough for already tracked markdown docs. To stop committing those files, they also need to be removed from the git index.
- Legacy config files should be normalized on read. Otherwise a renamed CLI can still expose stale, unsupported keys and make the current config surface look ambiguous.
