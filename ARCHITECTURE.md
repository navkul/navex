...
## What changed
- The registry is no longer treated as a historical list of every session the daemon has ever seen.
- The wrapper now passes its own launcher PID into Codex, and the `SessionStart` hook stores that PID on the tracked session.
- When the wrapped Codex process exits, the wrapper emits a `session-exit` event so the daemon can remove every session tied to that launcher immediately.
- Daemon startup now prunes any session whose launcher PID is missing or no longer alive before it rebuilds the overlay snapshot.
- The overlay snapshot now reflects all live tracked sessions, not only sessions already in `waiting`.

## Current behavior
- Exited interactive sessions are removed from on-disk state as soon as the wrapper process ends.
- Old stale entries from the pre-launcher-PID model are cleared the next time the daemon starts.
- The daemon's persisted JSON now behaves like live session state that can survive daemon restarts without accumulating dead rows.

# April 22, 2026 absolute-hook launcher pass

## What changed
- Hook setup now prefers the linked absolute `navex` binary when it resolves back to this repo's `dist/cli.js`.
- `navex install` falls back to `node + dist/cli.js` only when it cannot prove that the linked `navex` bin points at the current build.

## Current behavior
- Hook commands stay absolute so Codex does not depend on interactive shell PATH.
- The generated hook JSON is shorter and less repetitive on a normal `npm link` install.

# April 22, 2026 hook-path install pass

## What changed
- `navex install` now prints an exact `~/.codex/hooks.json` block with absolute `node` and `dist/cli.js` paths.
- The public README now points setup through that generated hook block instead of telling the user to copy the repo template blindly.

## Current behavior
- Hook installation is now path-stable even when Codex launches hooks without the user's normal shell PATH.

# April 22, 2026 navex cleanup pass

## What changed
- Removed the old-name runtime compatibility paths that were only kept for recovery.
- Navex now resolves to a single primary state root at `~/.navex/`.
- The remaining legacy handling is a narrow one-time migration path if an older machine still has only `~/.codex-beacon/`.

## Current behavior
- The daemon, helper, config, registry, and overlay snapshot all run from `~/.navex/`.
- The code no longer depends on stale `codex-beacon` bin aliases or `CODEX_BEACON_*` environment variables.

# April 22, 2026 navex rename pass

## What changed
- The primary package and CLI name are now `navex`.
- The runtime now defaults its local state root to `~/.navex/`.
- Legacy `~/.codex-beacon/` state is still recognized so the rename does not strand existing local data.
- Overlay presentation now carries a configurable `appDisplayName`, which feeds the menu-bar title and overlay header.
- The Swift helper source and build output now use the `NavexOverlay` name.
- Config loading now normalizes down to the supported Navex keys instead of echoing old notifier-era fields back through `config show`.

## Current behavior
- The public entrypoint for manager commands is now `navex`, while the user still launches tracked sessions through `codex`.
- Existing shell/hook setups can transition without a hard cutover because the runtime still accepts the older environment variable names and the package still exposes a compatibility `codex-beacon` bin.
